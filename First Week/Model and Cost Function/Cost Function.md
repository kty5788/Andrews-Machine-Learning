...
여기서 데이터가 잘 일치한다는 것은 x를 통한 h(x)의 값이 y값과 최대한 가까워 일치하는 것이다.
...
앞에 1/2m을 하는 이유는 1/m을 함으로써 평균값을 구해 더 쉽게 연산하기 위해서이고, 
1/2를 하는 것은 파라미터 θ0 , θ1을 최소화 하는 과정에서 같은 값의 과정(그니까 나중가면 할거를 지금 한다는 뜻)

솔직히 나도 1/2m의 정확한 이유를 잘 모르겠다.
**** 이해 , 연구를 하게 된다면 수정하겠습니다. 아는 분들은 좀 알려주세요.****
